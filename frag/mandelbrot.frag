...
void main() {
    ...
    float modulator = sin(u_time) * u_beat;
    for (int i=0; i<maxiter; i++) {
        //Perform complex number arithmetic
        p= vec2(p.x*p.x-p.y*p.y,(2.0+modulator)*p.x*p.y)+c;
        ...
        if (dot(p,p)>4.0){
            ...
            color = vec3(0.95 * modulator + .012*colorRegulator , 1.0, .2+.4*(1.0+sin(.3*colorRegulator)));
            break;
        }
    }
