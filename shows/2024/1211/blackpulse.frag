...
void main() {
  ...
  // // pulse
  s.x = cos(abs(s.y-s.x)/(sin(t*0.1)+2.)+a);
  s.y = abs(cos(s.x-s.y)/(tan(t*0.33)+2.)+a);
  ...
  if (mod(t*0.5, 13.) > 4.) {
    c -= step(.6+p,abs(sin(40.0*m_dist)))*.4*p;
  } else {
    c -= step(.6+p,abs(sin(40.0*m_dist)))*.7*p;
  }
