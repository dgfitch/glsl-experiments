...
void main() {
  ...
  // tv
  // s.x = cos(abs(r.x)*sin(t)+r.y*.01);
  // s.y = abs(cos(r.y)*sin(t)-r.x*.01);
  ...
  c = vec3(0.0);
	...
	for (int i = 0; i < 4; i++) {
    point[i] += sin(p*(2.1+float(i))) * .1;
    point[i] -= cos(p*(2.1+float(i))) * .1;
  }
  ...
  d = vec3(abs(sin(or.x+t) + tan(or.y+a)));
